# zolinga-intl Refactoring Plan — IMPLEMENTED ✅

**Status:** Implemented as of 2026-05-08. This document is the original plan; the actual implementation has some differences noted inline.
...
The following were added during implementation that were not in the original plan:

### New Model Classes (`src/Models/`)

The HTML/DOM logic was extracted into dedicated model classes rather than kept as methods on `GettextAbstract`:

| Class | Purpose |
|-------|---------|
| `GettextDocument` | Extends `DOMDocument`. Handles HTML loading, charset fixes, gettext mode meta tag, reindexing translatables. Contains static helpers: `parseGettextAttr()`, `getGettextMode()`, `calculateHash()`, `parseTranslatableKey()`, `updateGettextAttrHash()`. |
| `GettextElement` | Extends `DOMElement`, implements `GettextNodeInterface`. Handles element content translation with nested `<1>...</1>` placeholder support. |
| `GettextAttribute` | Extends `DOMAttr`, implements `GettextNodeInterface`. Handles attribute value translation. |
| `GettextNodeInterface` | Interface for translatable nodes with `gettextDomain`, `gettextHash`, `gettextString`, `gettextContext`, `descendantElements`, `translate()` contract. |

### New Enum: `GettextModeEnum`

**File:** `src/Types/GettextModeEnum.php`
...
Used in `Compiler::generateHtmlFile()` and `GettextDocument::$gettextMode` property.

### New Method: `Extractor::splitContextInPotFile()`

Post-processes `.pot` files to split `"context\x04message"` msgid entries into proper `msgctxt`/`msgid` pairs. Called after each `extract*PotFile()` method.

### `GettextDomain::$localesWithPO`

Added a `localesWithPO` property that discovers locales with existing `.po` files. Used by `Compiler` and `JavascriptCompiler` instead of `$this->locales` to only process locales that have translations.

### `GettextCli` parameter change

Uses `--domains` parameter (comma-separated) instead of `--module`.

### `I18nService` creates domains for ALL modules

Domains are created for all modules regardless of whether they have a `locale/` directory. The `checkRequirements()` in `GettextAbstract` will create the directory if missing.

### `Compiler::generateHtmlFile()` rewritten

The old `buildFileDictionary()` + `translateHtmlStrings()` approach was replaced with a direct `GettextDocument`-based approach:
- Source and target documents are loaded as `GettextDocument` instances
- Translation iterates over `$targetDoc->translatables`, looks up matching entries in `$sourceDoc->translatables`
- Uses `$node->translate()` on each translatable node
- No more separate dictionary building phase

### `Extractor::fixEscapedSlashesInPotFile()`

Added post-processing to unescape `\/` back to `/` in .pot files. `xgettext` escapes forward slashes as `\/`, which causes `msgmerge` to fail with "invalid control sequence" on `<\/`. This method is called after each `extract*PotFile()` method.

### `GettextAbstract::findExistingPoLocales()` removed

The duplicated `findExistingPoLocales()` method was removed from `GettextAbstract` — it now uses `$this->domain->localesWithPO` from `GettextDomain` instead.
...
**File:** `src/Types/FileTypes.php` ✅ IMPLEMENTED
...
**File:** `src/Gettext/GettextDomain.php` ✅ IMPLEMENTED (with additions)
...
**File:** `src/Exceptions/GettextDomainException.php` ✅ IMPLEMENTED
...
**File:** `src/I18nService.php` ✅ IMPLEMENTED
...
### 2.2 `getGettextDomains()` logic ✅ IMPLEMENTED

1. Build `GettextDomain` objects for all modules (regardless of whether they have a `locale/` directory):
...
### 2.3 `zolinga.json` update ✅ IMPLEMENTED
...
### 3.1 `GettextAbstract` — accept `GettextDomain` instead of module path ✅ IMPLEMENTED
...
- All existing HTML/DOM methods (`loadHtmlFile`, `findTranslatables`, `parseGettextAttr`, `getGettextMode`, `calculateHash`) have been **moved** to `GettextDocument` and related Model classes — they are no longer on `GettextAbstract`

### 3.2 `Extractor` — split extraction by file type into separate .pot files ✅ IMPLEMENTED
...
- **New `splitContextInPotFile()`**: post-processes each `.pot` file to split `"context\x04message"` msgid entries into proper `msgctxt`/`msgid` pairs
- `extractInBatches()` and `getExtractCmd()`: unchanged, just parameterized with target pot file
- `extractHtmlStrings()` and `makePhpLine()`: updated to use `GettextDocument` model instead of DOM methods on `GettextAbstract`; `$relativeFile` now uses `$api->fs->toZolingaUri($file)` instead of `$this->modulePath` — consistent with `exec()` now using `chdir(ROOT_DIR)`

### 3.3 `Compiler` — compile .po → .mo, translate HTML using static domain ✅ IMPLEMENTED
...
**Key changes:**
- `compileLanguagePoFiles()`: for each locale in `$this->domain->localesWithPO`, runs `msgmerge {lang}.po templates/server.pot` to a temporary `.po` in the system temp directory, then `msgfmt` → `{domain}.mo`. This produces an `.mo` containing only the PHP strings (intersection of full translations with `server.pot`).
- New `compileStaticPoFiles()`: for each locale in `$this->domain->localesWithPO`, runs `msgmerge {lang}.po templates/static.pot` to a temporary `.po` in the system temp directory, then `msgfmt` → `{domain}.static.mo`.
...
- `translateHtmlFiles()`: uses `GettextDocument::getGettextMode()` to check mode, then calls `generateHtmlFile()` for each locale in `$this->domain->localesWithPO` (excluding `en_US`)
- `generateHtmlFile()`: **completely rewritten** — uses `GettextDocument` model directly:
  - Loads source and target documents as `GettextDocument` instances
  - Iterates over `$targetDoc->translatables`, looks up matching entries in `$sourceDoc->translatables`
  - Uses `dgettext($domain . '.static', $string)` for translation
  - Calls `$node->translate($translation, $template->descendantElements)` on each node
- `buildFileDictionary()`, `translateHtmlStrings()`, `normalizeString()`: **removed** — replaced by `GettextDocument` model approach
- `mkFileName()`: **unchanged**

### 3.4 `JavascriptExtractor` — removed, merged into `Extractor` ✅ IMPLEMENTED
...
### 3.5 `JavascriptCompiler` — refactored to use `GettextDomain` ✅ IMPLEMENTED
...
### 4.1 `initGettext()` — domain binding once, locale env always ✅ IMPLEMENTED
...
### 4.2 No compile-time methods in `LocaleService` ✅ IMPLEMENTED
...
### 5.1 `extract()` — use `$api->i18n->getGettextDomains()` ✅ IMPLEMENTED

```php
public function extract(RequestResponseEvent $event): void {
    $domains = $api->i18n->getGettextDomains();

    // Filter by --domains if specified (comma-separated)
    if ($domainsFilter = $event->request['domains'] ?? null) {
        $filterList = array_map('trim', explode(',', $domainsFilter));
...
### 5.2 `compile()` — use `$api->i18n->getGettextDomains()` ✅ IMPLEMENTED
...
### 5.3 Remove `getTargetModules()` ✅ IMPLEMENTED
...
| `src/Types/GettextModeEnum.php` | Enum for gettext modes: translate, cherry-pick, replace |
...
| `src/Models/GettextDocument.php` | DOMDocument subclass for HTML loading, charset fixes, translatable indexing |
| `src/Models/GettextElement.php` | DOMElement subclass for element content translation with nested placeholder support |
| `src/Models/GettextAttribute.php` | DOMAttr subclass for attribute value translation |
| `src/Models/GettextNodeInterface.php` | Interface for translatable nodes |
...
| File | Changes |
|------|---------|
| `zolinga.json` | Add `i18n` service listener (no config keys — `default` domain is hard-coded); version bumped to `2.0` |
| `src/Gettext/GettextAbstract.php` | Constructor takes `GettextDomain`; `findFiles()` scans multiple folders, uses `FileTypes` constants, ignores dot-files/backup extensions; remove `$modulePath`/`$moduleName`; remove all HTML/DOM methods (moved to Models) |
| `src/Gettext/Extractor.php` | Split extraction into `extractServerPotFile()`/`extractClientPotFile()`/`extractStaticPotFile()` → separate `.pot` files; add `mergePotFiles()`; add `splitContextInPotFile()`; use `GettextDocument` model for HTML extraction |
| `src/Gettext/Compiler.php` | Use `GettextDomain`; add `compileStaticPoFiles()`; rewrite `generateHtmlFile()` to use `GettextDocument` model; remove `buildFileDictionary()`, `translateHtmlStrings()`, `normalizeString()` |
...
| `src/GettextCli.php` | Use `$api->i18n->getGettextDomains()`; remove `getTargetModules()`; single-phase extract/compile; use `--domains` parameter (comma-separated) |
...
|-------|-------------|--------------|
| `GettextDocument` | `__construct()` | DOMDocument loading with charset fix (moved from `GettextAbstract::loadHtmlFile()`) |
| `GettextDocument` | `reindex()` | XPath `//@gettext` query, rebuild `$translatables` array (moved from `GettextAbstract::findTranslatables()`) |
| `GettextDocument` | `parseGettextAttr()` | Parse `[domain:](.\|attr)[#hash]` tags (moved from `GettextAbstract::parseGettextAttr()`) |
| `GettextDocument` | `getGettextMode()` | Read `<meta name="gettext" content="...">` (moved from `GettextAbstract::getGettextMode()`) |
| `GettextDocument` | `calculateHash()` | SHA1 first 6 chars (moved from `GettextAbstract::calculateHash()`) |
...
| `Extractor` | `extractHtmlStrings()` | DOM parse → fake PHP `_()`/`dgettext()` calls; now uses `GettextDocument` model |
...
| `Compiler` | `generateHtmlFile()` | Uses `GettextDocument` model; iterates `$targetDoc->translatables`, looks up in `$sourceDoc->translatables`, calls `$node->translate()` |
